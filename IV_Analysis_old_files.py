...
for filename in src_files:
    ...
    CurrentIn = np.linspace(-300, 300, len(abf.sweepList))  # ! I create the current input data. Use data[1] for ABF2.0. No data[1] with WinWCP converted ABF
    ...
    PulseStart = int(1.50*abf.dataRate)                     # Seconds. Depends on the protocol. we'll be in a dict soon. 1.5 before. 1.65 now?!
    PulseEnd = int(2.50*abf.dataRate)
    ResistancePulseStart = int(4.50*abf.dataRate)
    RPulseInterval = int(0.500*abf.dataRate)
    ResistancePulseEnd = int(5.00*abf.dataRate)
    SagFrame = int(0.150*abf.dataRate)
    ...
    for sweepNumber in abf.sweepList:
        ...
        ResistanceSweep[sweepNumber] = (np.mean(abf.sweepY[ResistancePulseStart:ResistancePulseEnd])
                                       ...
                                       /(np.mean(abf.data[1][ResistancePulseStart:ResistancePulseEnd])*1e-3)  # real values in data var.
    ...
    c = abf.data[1][PulseStart:PulseEnd]
    ...
    SteadyResistance = (VBaseline[0]-Vsteady[0])/-(np.mean(abf.data[1][PulseEnd - AverageOffOnSet:PulseEnd])*1e-3)
    ...
    plt.hlines(results_half[1]*-1,(1.50*20000+results_half[2])/20000,(1.50*20000+results_half[3])/20000, color='red')
    ...
    plt.axis([1.35, 2.70, limitmin, limitmax])  # plt.axis([xmin,xmax,ymin,ymax])
    ...
    ax1.plot(abf.sweepX, abf.data[1],'k')#[len(abf.sweepX) * 8:len(abf.sweepX) * 9], 'b', linewidth=1)
    ...
    plt.axis([1.5, 2.85, np.amin(abf.data[1][len(abf.sweepX) * 8:len(abf.sweepX) * 9]),
              np.amax(abf.data[1])]) # plt.axis([xmin,xmax,ymin,ymax])
    plt.gcf()
    plt.draw()
